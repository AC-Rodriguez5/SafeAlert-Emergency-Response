...
router.patch('/:id/offline', async (req, res) => {
    try {
        const alert = await Alert.findByIdAndUpdate(req.params.id, { isOnline: false }, { new: true }).lean();
        if (!alert) {
            return res.status(404).json({ error: 'Alert not found' });
        }
        const io = req.app.get('io');
...
router.patch('/:id/status', async (req, res) => {
    try {
        ...
        const alert = await Alert.findByIdAndUpdate(req.params.id, updateData, { new: true }).lean();
        if (!alert) {
            return res.status(404).json({ error: 'Alert not found' });
        }
